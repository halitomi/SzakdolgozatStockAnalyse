...
def yf_tech_analysis(ticker: str, period: str = "1y"):
    ...
    print(f"[TOOL CALL] yf_tech_analysis used with ticker={ticker}")
    stock = yf.Ticker(ticker)
    history = stock.history(period=period)

    if history.empty:
        raise ValueError(f"No data fetched for ticker {ticker} with period {period}.")
    ...
    if df.empty:
        raise ValueError("No valid data available after processing technical indicators.")
    ...
    return {
        ...
        "sma_50": df['trend_sma_50'].iloc[-1] if 'trend_sma_50' in df else None,
        "sma_200": df['trend_sma_200'].iloc[-1] if 'trend_sma_200' in df else None,
        "rsi": df['momentum_rsi'].iloc[-1] if 'momentum_rsi' in df else None,
        "macd": df['trend_macd_diff'].iloc[-1] if 'trend_macd_diff' in df else None,
        "bollinger_hband": df['volatility_bbhi'].iloc[-1] if 'volatility_bbhi' in df else None,
        "bollinger_lband": df['volatility_bbli'].iloc[-1] if 'volatility_bbli' in df else None,
        "atr": df['volatility_atr'].iloc[-1] if 'volatility_atr' in df else None,
